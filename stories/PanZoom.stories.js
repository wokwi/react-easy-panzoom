...
const Box = ({ children }) => (
  <div style={{ border: 'solid 1px red', padding: 8, left: 6 }}>{children}</div>
);
  ...
  return (
    <DefaultPanZoom ref={ref}>
      <div style={{ padding: 10, border: '2px solid red' }}>
        Move me then <button onClick={onClick}>AutoCenter</button>
      </div>
    </DefaultPanZoom>
  );
};
...
storiesOf('react-easy-panzoom', module)
  ...
  .add('Basic', () => (
    ...
    >
      <Box>This div can be panned</Box>
    </DefaultPanZoom>
  ))
  .add('Prevent pan', () => <PanZoomPreventPan />)
  .add('No state update on pan', () => (
    <DefaultPanZoom maxZoom={Infinity} noStateUpdate={boolean('Disable state update on pan', true)}>
      <Box>This div can be panned</Box>
    </DefaultPanZoom>
  ))
  .add('Bounding box', () => (
    <DefaultPanZoom
      maxZoom={Infinity}
      boundaryRatioHorizontal={number('Horizontal boundary ratio', 0.8, {
        range: true,
        min: -1,
        max: 2,
        step: 0.1,
      })}
      boundaryRatioVertical={number('Vertical boundary ratio', 0.8, {
        range: true,
        min: -1,
        max: 2,
        step: 0.1,
  ...
  .add('Control UI', () => (
    <PanZoomControlUI
      ...
      enableBoundingBox
      realPinch={boolean('Enable real pinch', false)}
      zoomInSpeed={number('Controlled zoom in speed', 1, {
        range: true,
        min: 0.1,
        max: 2,
        step: 0.1,
      })}
      zoomOutSpeed={number('Controlled zoom out speed', 1, {
        range: true,
        min: 0.1,
        max: 2,
        step: 0.1,
  ...
  .add('Disable Double Click Zoom Event', () => (
    ...
    >
      <Box>Double Click should not zoom</Box>
    </DefaultPanZoom>
  ))
  .add('Disable Scroll Zoom Event', () => (
    <DefaultPanZoom maxZoom={Infinity} disableScrollZoom={boolean('Disable Scroll Zoom', true)}>
      <Box>Scroll should not zoom</Box>
    </DefaultPanZoom>
  ))
  .add('onStateChange handler', () => {
    return (
      ...
      >
        <Box>Open the console then move me</Box>
      </DefaultPanZoom>
    );
  })
  .add('autoCenter animate option', () => (
    <AutoCenterDemo animate={boolean('Animate auto center', true)} />
  ));
